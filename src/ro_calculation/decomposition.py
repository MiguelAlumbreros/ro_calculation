...
1. **Windows** — which dates the regulator averages over (`averaging_window`) and
   which contract those closes belong to (`delivery_window`).
2. **Fixed / floating split** — for a given `asof`, how much of each window has
   already happened (fixed, an observed mean) versus is still to come (floating,
   normally the last close carried forward, or the PFC's tenor average when one is
   supplied — see `pfc.py`), and the day-weighted blend of the two.
"""
...
def averaging_window(target_quarter: str, commodity: str, leg: str) -> tuple[date, date]:
    """Regulatory averaging window per docs/product_open_dates.txt.
    ...
    - M1, M2, M3 (monthly legs):
        * EUA: same window as Q (the full previous quarter, day 1 of its
          ...
          share this window because EUA is averaged at the quarterly level.
    ...
    The end-of-window day 20 reflects the regulator's cut-off before quarter end;
    the period from day 21 to month-end is excluded from the average.
    """
...
def _product_rows(df: pd.DataFrame, delivery_start: date, delivery_end: date) -> pd.DataFrame:
    ...
    Matched on the raw datetime64 arrays rather than through Series comparison. Both
    columns are already datetime64 after `normalize_prices`, so `to_numpy` is a view and
    this selects the same rows with none of the per-operation overhead, which matters
    because every leg of every quarter runs it over the full exchange frame.
    """
    matches = (
...
def _decompose_product(
    ...
    Window-day counts use calendar-day differences (win_end - win_start, etc.),
    not business days. This is approximate vs. the exchange's actual settlement
    days but is sufficient for the pct_open weighting.

    The floating portion is normally the last observed futures close held flat. When
    `pfc_df` is given, it is used instead: the mean of the PFC's hourly `commodity`
    column over the contract's delivery window (`delivery_start`..`delivery_end`).
    The fixed (already-observed) portion is never sourced from the PFC.
    """
...
# Everything the decomposition reads once an exchange has been picked. Selecting these
# here means the per-leg delivery-window masks copy four columns rather than all fifteen.
...
def _decompose_with_fallback(
    ...
    the leg is resolved on its own delivery window anyway — fixed stays empty (there is
    no futures history for a leg no exchange has ever quoted), float comes from the PFC.
    This is deliberately preferred over inheriting the parent leg's often-fully-elapsed
    average, since the PFC has genuine per-leg granularity that far-dated futures lack.
    ...
    `gather_price_components` splits them once so that a commodity's five legs share one
    split rather than each rebuilding it.
...
# Columns the decomposition compares against a scalar to pick out one exchange's rows
# (see `_rows_for_exchange` and `EXCHANGE_ROW_FILTER`). Held as strings, every such
# comparison rescans the whole column for missing values; held as categories it is an
# integer compare against the codes. All five have at most a handful of distinct values.
...
def _is_normalized(df: pd.DataFrame) -> bool:
    ...
    if not all(isinstance(df[c].dtype, pd.CategoricalDtype)
        ...
        return False
    # The categories are built after the strip filter, so they are exactly the strips
    # present. Reading them off the dtype avoids scanning the column on every call.
...
def normalize_prices(df: pd.DataFrame) -> pd.DataFrame:
    ...
    An already-normalised frame is handed straight back. `pipeline.run_asof` normalises
    once and then fans out over quarters and as-of dates, and re-filtering and re-parsing
    a few hundred thousand rows for every pair was the largest single cost of a
    multi-quarter run.
    """
...
def gather_price_components(
    ...
    A commodity with a single leg (EUA/M3) has no separate leg row: its total row
    is already identical to that leg (weight 1.0), so the leg row would be redundant.

    `df_power`/`df_gas`/`df_eua` need only be filtered to their `commodity`; they may
    contain rows from multiple exchanges/areas/products (e.g. OMIP and EEX power, or
    MIBGAS and EEX gas) since `_decompose_with_fallback` picks the right exchange per
    leg via `EXCHANGE_PRIORITY`.

    `pfc_df` is the loaded hourly Price Forward Curve (see `pfc.load_pfc`). When given,
    it replaces the floating (still-open) portion of every leg's price for every
    commodity present in its columns; the fixed (already-observed) portion still comes
    from futures. Pass `None` (the default) to use futures for both fixed and float.
    """
    if asof is None:
        asof = date.today()
    ...
    for commodity in ("power", "gas", "EUA"):
        ...
        if all(leg_is_primary_direct):
            ...
            # The distinct sources actually used across legs, in leg order, instead of
            # collapsing to the generic "mixed" label.
