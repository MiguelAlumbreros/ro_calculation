"""Assemble the constant set for a target quarter from the sheets shipped
in the package.

`constants_for("q3_26")` returns the full mapping `formula.calculate_ro` needs. It
merges three sources, in this order:

1. `config.STATIC_CONSTANTS`     — taxes, levies, storage canons and gas tolls.
2. `ro_parameters.xlsx`          — the valores propios of the instalación tipo, one row
                                   per (Codigo, Ano).
3. `IVPEE.xlsx`                  — the IVPEE rate, one row per period.

A quarter the IVPEE sheet does not cover raises `KeyError`; add a row for it. A year the
parameters sheet does not cover falls back to the last year on file and warns; add the
row from the orden de parámetros retributivos to silence it.
"""
...
# Column in IVPEE.xlsx holding the rate to run the RO calculation at. It is not
# always the rate levied: `tipo_efectivo_pct` in the same sheet holds that one.
...
_PARAMETER_COLUMNS: dict[str, str] = {
    ...
    "VAC":        "V_AC",         # ajuste de mercado, used as a tanto por uno. Check the
                                  # scaling before entering a non-zero value.
...
def _ivpee_periods(path: Path, mtime_ns: int) -> tuple[tuple[pd.Timestamp, pd.Timestamp, float], ...]:
    ...
    Pass the file's current `st_mtime_ns` as `mtime_ns`; editing the sheet then takes
...
def _valores_propios(
    ...
    Pass the file's current `st_mtime_ns` as `mtime_ns`, as for `_ivpee_periods`.
    """
...
def ivpee_for_quarter(target_quarter: str) -> float:
    ...
    the quarter, or if the row covering it has no rate in the RO column.
    """
    path = ivpee_path()
    if not path.exists():
...
def installation_parameters(
    ...
    A year with no row of its own falls back to the latest earlier year on file and
    warns. An installation type absent from the sheet raises `KeyError`.
    """
